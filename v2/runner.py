...
from v2.runner_interface import backport, clean_backport, get_benchmarks, BenchmarkRunner, CorePool, WorkOrder
from v2.logging_subprocess import call
from v2.runner_policy import BASELINE_BUILD_CORE_COUNT, NUM_CORES, OPPORTUNISTIC_BUILD_CORE_COUNTS, POOL_SLACK
from v2.workspace import (
    BUILD_COMPLETED_ROOT, DATE_FMT, MUTATION_LOCK, RUN_LOG_ROOT, RUNNER_STATE_ROOT,
    STOP_FILE, SWEEP_CADENCE, SWEEP_START)
...
class Runner:
    def __init__(self):
        ...
        self._core_pool = CorePool(POOL_SLACK)
    ...
    def run(self):
        ...
        # self.worker_fn(0)
        num_workers = 8  # TODO: tune.
        with multiprocessing.dummy.Pool(num_workers) as pool:
            pool.map(self.worker_fn, range(num_workers), 1)

    def worker_fn(self, worker_id: int):
        # TODO: bisection and test loop will have higher priority.
    ...
    def measure(self):
        ...
        try:
            ...
            #     python -c "from torch.utils_backport.benchmark import Timer; print(Timer('torch::empty({1});', language='c++').collect_callgrind(repeats=10))"
            #     """,
            #     shell=True,
            #     # check=True,
            #     conda_env=conda_env,
            #     # per_line_fn=lambda l: print(l),
            ...
            for label, auto_labels, timer_args in self._benchmarks[:10]:
                # TODO: Figure out when jit models will work. (And fix enum compat.)
                if auto_labels.runtime.value == "TorchScript":
                    continue
            ...
            results = BenchmarkRunner(work_items=tuple(work_orders), core_pool=self._core_pool, display_progress=True).run()
            import pdb
            pdb.set_trace()
